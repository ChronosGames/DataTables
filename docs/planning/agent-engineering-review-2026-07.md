...
1. **覆盖面较完整**：当前 Agent 指令包含项目定位、核心组件、生成器结构、常用命令、Unity 说明、测试命令和兼容 API，对新 Agent 有较好的启动帮助。
...
4. **保留兼容信息**：旧回调 API、`GetDataTable*`、`CreateDataTable*` 等兼容入口被提示为旧路径，能减少 Agent 破坏性重构风险。
...
| API 名称漂移 | Agent 文档示例使用 `EnableMemoryManagement(50)`，README 强调 `EnableEstimatedMemoryBudget(50)` | Agent 可能在新代码中偏向旧别名或不准确命名 | Agent 文档应以 README 当前推荐 API 为准，旧别名放兼容区 |
...
3. **同步当前推荐 API**
   ...
   - `EnableMemoryManagement`、旧回调 API、`GetDataTable*` 放在兼容章节。
   - 验收：新代码示例不再优先展示旧别名。
