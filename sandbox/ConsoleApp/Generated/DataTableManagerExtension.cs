...
namespace ConsoleApp
{
    ...
    public static class DataTableManagerExtension
    {
        ...
        {
            int done = 3;
            manager.CreateDataTable<DTDataTableSample>(() => { if (--done == 0) onCompleted?.Invoke(); });
            manager.CreateDataTable<DTDataTableSplitSample>("x001", () => { if (--done == 0) onCompleted?.Invoke(); });
            manager.CreateDataTable<DTDataTableSplitSample>("x002", () => { if (--done == 0) onCompleted?.Invoke(); });
