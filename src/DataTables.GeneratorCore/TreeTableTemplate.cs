...
namespace DataTables.GeneratorCore
{
    ...
    private string BuildTreeApi()
    {
        ...
        WL($"    public static IReadOnlyList<{row}> GetRootsStatic(string dataTableName) => DataTableManager.GetDataTableInternal<{table}>(dataTableName)?.GetRoots() ?? Array.Empty<{row}>();");
        WL($"    public static IReadOnlyList<{row}>? GetChildrenStatic(string id) => GetChildrenStatic(string.Empty, id);");
        WL($"    public static IReadOnlyList<{row}>? GetChildrenStatic(string dataTableName, string id) => DataTableManager.GetDataTableInternal<{table}>(dataTableName)?.GetChildren(id);");
        WL($"    public static {row}? GetParentStatic(string id) => GetParentStatic(string.Empty, id);");
        WL($"    public static {row}? GetParentStatic(string dataTableName, string id) => DataTableManager.GetDataTableInternal<{table}>(dataTableName)?.GetParent(id);");
        WL($"    public static IEnumerable<{row}> TraverseDepthFirstStatic(string id) => TraverseDepthFirstStatic(string.Empty, id);");
        WL($"    public static IEnumerable<{row}> TraverseDepthFirstStatic(string dataTableName, string id) => DataTableManager.GetDataTableInternal<{table}>(dataTableName)?.TraverseDepthFirst(id) ?? Array.Empty<{row}>();");
        WL("    #endregion");
        return sb.ToString();
    }
